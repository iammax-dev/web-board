...
import com.max.board.web.config.auth.dto.OAuthAttributes;
import com.max.board.web.config.auth.dto.SessionUser;
import com.max.board.web.domain.user.User;
import com.max.board.web.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // OAUTH2 로그인 진행시 키가 되는 필드값을 이야기함 (PK와 같은 의미), 구글 기본코드 - sub
    ...
    private User saveOrUpdate(OAuthAttributes attibutes) {
        User user = userRepository.findByEmail(attibutes.getEmail())
                .map(entity -> entity.update(attibutes.getName(), attibutes.getPicture())).orElse(attibutes.toEntity());
